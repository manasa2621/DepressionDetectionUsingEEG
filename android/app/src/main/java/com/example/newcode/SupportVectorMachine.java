...
public class SupportVectorMachine {
    ...
    public void train(List<RealVector> negative, List<RealVector> positive, int precisionSteps) throws Exception {
    ...
    private double findMin(List<RealVector> merged) {
        ...
        for (RealVector vector : merged) {
            for (int i = 0; i < vector.getDimension().length; i++) {
    ...
    private Option getMinimalOption(HashMap<Double, Option> options) throws Exception {
        if (options.size() == 0) {
            throw new Exception();
        }
    ...
    private RealVector addToVector(RealVector w, double v) {
        ...
        for (int i = 0; i < w.getDimension().length; i++) {
    ...
    public TypeClassification classify(RealVector feature) {
        if(this.w == null) {
            return null;
        }

        double result = feature.dotProduct(this.w) + this.b;
        return result < 0 ? TypeClassification.NEGATIVE : TypeClassification.POSITIVE;
    }
}